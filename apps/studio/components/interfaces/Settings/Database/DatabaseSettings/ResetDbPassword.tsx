...
export const ResetDbPassword = ({ disabled = false }) => {
  return (
    <PageSection id="database-password">
      <PageSectionMeta>
        <PageSectionSummary>
          <PageSectionTitle>{$t('Database password')}</PageSectionTitle>

          <PageSectionDescription>
            {$t('Used for direct Postgres connections')}
          </PageSectionDescription>
        </PageSectionSummary>
      </PageSectionMeta>
      <PageSectionContent>
        <Card>
          <CardContent className="flex flex-row items-center gap-x-2 justify-between">
            <div className="space-y-0.5">
              <h3 className="text-sm text-foreground">{$t('Reset database password')}</h3>
