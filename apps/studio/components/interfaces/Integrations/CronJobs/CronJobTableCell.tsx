        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-44 space-y-1">
            <Tooltip>
              ...
              <TooltipContent>
                {$t(
                  'Manual runs execute the command immediately and will not appear in the cron jobs table.'
                )}
              </TooltipContent>
            </Tooltip>
            <DropdownMenuItem
    ...
    return (
      <Dialog open={showToggleModal} onOpenChange={setShowToggleModal}>
        ...
        >
          ...
          <DialogSection>
            <p className="text-sm">
              {$t('Are you sure you want to')} {active ? 'disable' : 'enable'}{' '}
              {$t('the cron job "')}
              {jobname}
              "?{' '}
            </p>
        ...
        <div className={cn('w-full flex items-center text-xs')}>
          {['latest_run', 'next_run'].includes(col.id) ? (
            !hasValue ? (
              ...
              <p className="text-foreground-lighter">{$t('Job has not been run yet')}</p>
            ) : col.id === 'next_run' && !formattedValue ? (
              <p className="text-foreground-lighter">{$t('Unable to parse next run for job')}</p>
            ) : (
              <TimestampInfo
            ...
            <HoverCard openDelay={0} closeDelay={0}>
              ...
              <HoverCardContent
                ...
                onClick={(e) => e.stopPropagation()}
              >
                <p className="text-xs font-mono px-2 py-1 border-b">{$t('Command')}</p>
                <CodeBlock
                  hideLineNumbers
                  language="sql"
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex items-center gap-x-2 w-full">
                <Edit size={12} />
                <span>{$t('Edit job')}</span>
              </div>
            </TooltipTrigger>
            {!jobname && (
              <TooltipContent side="right" className="w-56">
                {$t(
                  'This cron job doesn’t have a name and can’t be edited. Create a new one and delete this job.'
                )}
              </TooltipContent>
            )}
          </Tooltip>
