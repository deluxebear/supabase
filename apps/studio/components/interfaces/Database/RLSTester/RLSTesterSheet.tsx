  ...
  return (
    <>
        ...
        <SheetContent className="w-[600px]! flex flex-col gap-y-0">
          <SheetHeader>
            <SheetTitle className="flex items-center gap-x-4">
              <span>{$t('What data can my users access?')}</span>
              <FeaturePreviewBadge featureKey={LOCAL_STORAGE_KEYS.UI_PREVIEW_RLS_TESTER} />
            </SheetTitle>
            <SheetDescription>
              
                                        {$t('See what data a user is allowed to read or modify based on your RLS policies')}
                                      </SheetDescription>
          </SheetHeader>
          ...
          <div className="grow overflow-y-auto flex flex-col">
            ...
            {blockedReason?.type === 'multiple-statements' ? (
              <div className="p-4">
                <Admonition
                  type="warning"
                  title={$t('Only a single SQL statement is supported')}
                  description={$t('Remove any additional statements and run the query again.')}
                />
              </div>
              ...
              <div className="p-4">
                <Admonition
                  ...
                  description={$t('Support for testing UPDATE and DELETE statements will be available soon.')}
                />
              </div>
            ) : parseQueryError ? (
              <div className="p-4">
      ...
      <ConfirmationModal
        ...
        size="medium"
        loading={isLoading}
        title={$t('Confirm to run this query')}
        ...
        {sandboxEnabled && (
          <>
            <p className="text-sm">
                                        ...
                                        {$t('We highly recommend using the sandbox to set up an ephemeral database environment for testing insert, update, or delete queries.')}
                                      </p>
            <Button
              variant="default"
              className="mt-2"
