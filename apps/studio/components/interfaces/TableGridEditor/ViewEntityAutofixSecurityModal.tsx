...
export const ViewEntityAutofixSecurityModal = ({
  ...
  const { mutate: execute } = useExecuteSqlMutation({
    onSuccess: async () => {
      toast.success($t('View security changed successfully'))
  ...
  return (
    <ConfirmationModal
      ...
      size="xlarge"
      title={$t('Confirm autofixing view security')}
      confirmLabel="Confirm"
      onCancel={() => setIsAutofixViewSecurityModalOpen(false)}
      onConfirm={() => handleConfirm()}
    >
      <p className="text-sm text-foreground-light">
        {$t('Setting')} <code>security_invoker=on</code>{' '}
        {$t(
      ...
      <div className="flex items-center gap-8 mt-8">
        <div className=" border rounded-md w-1/2">
          <div className="p-4 pb-0 bg-200 font-mono text-sm font-semibold">
            {$t('Existing query')}
          </div>
