...
order by 1 asc`
  ...
  return async () => {
    ...
    const rows = (data?.result ?? []).map((row: any) => ({
...
const buildServiceHealth: Builder = (projectRef, params) => {
  ...
  return async () => {
    ...
    queries.forEach(({ table }, i) => {
      const rows = results[i].data?.result ?? []
    ...
    const rows = [...buckets.values()]
      ...
      .map((row) => {
        const merged: Record<string, any> = { timestamp: row.timestamp }
...
const buildAuthMetrics: Builder = (projectRef, params) => {
  ...
  return async () => {
    ...
    const rows = windows.map(({ period }, i) => {
      const authRow = results[i * 2].data?.result?.[0] ?? {}
      const edgeRow = results[i * 2 + 1].data?.result?.[0] ?? {}
      return {
        period,
