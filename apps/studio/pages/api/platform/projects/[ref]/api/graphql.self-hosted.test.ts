...
describe('POST /platform/projects/{ref}/api/graphql (plain self-hosted, zero-break)', () => {
  it('proxies to global SUPABASE_URL with global service key; resolver never called', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe('http://localhost:8000/graphql/v1')
    expect(fetchMock.mock.calls[0][1].headers.apikey).toBe('global-service-key')
    expect(fetchMock.mock.calls[0][1].headers.Authorization).toBe('Bearer global-anon-key')
