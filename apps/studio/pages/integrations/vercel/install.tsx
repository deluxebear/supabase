          ...
          {showLoadingState ? (
            ...
            <div className="flex flex-col gap-5">
              ...
              {missingParams.length > 0 && (
                <Admonition
                  type="warning"
                  title={$t('Missing Vercel installation details')}
...
const InstallationLoadingState = () => (
  <div className="flex flex-col gap-5">
    ...
    <section className="space-y-2" aria-label={$t('Organization loading')}>
...
function OrganizationSelect({
  ...
  return (
    <section className="space-y-2" aria-label={$t('Organization')}>
      ...
      >
        <SelectTrigger size="small" aria-label={$t('Supabase organization to install Vercel into')}>
          <SelectValue placeholder={$t('Choose an organization')} />
        </SelectTrigger>
        <SelectContent>
          {organizations.map((org) => (
            <SelectItem key={org.slug} value={org.slug} className="text-xs">
              <div className="flex items-center gap-2">
                ...
                {installed[org.slug] && <Badge className="flex-none!">{$t('Installed')}</Badge>}
              </div>
            </SelectItem>
          ))}
        </SelectContent>
