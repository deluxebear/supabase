...
const CodePage = () => {
  ...
  const { mutate: deployFunction, isPending: isDeploying } = useEdgeFunctionDeployMutation({
    onSuccess: () => {
      toast.success($t('Successfully updated edge function'))
      ...
      {isErrorLoadingFiles && (
        <div className="flex flex-col items-center justify-center h-full bg-surface-200">
          <div className="flex flex-col items-center text-center gap-3 max-w-md">
            ...
            <h3 className="text-md mt-4">{$t('Failed to load function code')}</h3>
            ...
            <div className="text-sm text-foreground-light border-t border-border-muted pt-3 mt-2">
              <p className="font-medium mb-2">{$t('To resolve this issue:')}</p>
              <ol className="text-left space-y-1">
                <li>{$t('1. Update to the latest Supabase CLI version')}</li>
                <li>
                  {$t('2. Redeploy your function using:')}{' '}
                  <code className="text-xs bg-muted px-1 py-0.5 rounded">
                    {$t('supabase functions deploy')}
                  </code>
                </li>
                <li>
                  {$t('3. Or use the')}{' '}
